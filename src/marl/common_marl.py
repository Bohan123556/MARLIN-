...
def parse_arguments(llm_version: bool):
    ...
    parser.add_argument(
        "--episode-total-limit", type=int, default=None, help="Total episode limit (default: None)"
    )
...
def get_base_config(
    ...
    config = (
        ...
        )  # Num of envs per thread -- for now using the LLMStochasticSampler this must stay at 1. TODO Fix indexing somehow to support multiple envs per worker
        .training(
            lr=lr,
            kl_coeff=0.01,
            clip_param=0.2,
...
def go(config, total_timesteps, wandb_proj):
    ...
    tuner = tune.Tuner(
        ...
        run_config=air.RunConfig(
            ...
            checkpoint_config=CheckpointConfig(checkpoint_frequency=1, checkpoint_at_end=True),
        ),
    )
    results = tuner.fit()
    return results
...
class HybridEvaluationCallbacks(DefaultCallbacks):
    def on_episode_end(  # Use this to checkpoint specific episodes?
        ...
        **kwargs,
        ...
        eps += 1
        if eps % 10 == 0:
            path = os.path.join(os.path.abspath("./models/hybrid/"), f"episode_{eps}")
    ...
    def on_episode_end(  # Use this to checkpoint specific episodes?
        ...
        **kwargs,
        ...
        if eps % 10 == 0:
            with file_lock:
                try:
                    path = os.path.join(os.path.abspath("./models/marl/"), f"episode_{eps}/")
                    if not os.path.exists(path):
                        os.mkdir(path)
                    num_dirs = len(os.listdir(path))
                    final_path = os.path.join(path, f"trial_{num_dirs}/")
                    ...
                    try:
                        ...
                        final_path = os.path.join(
                            path, f"trial_{num_dirs}_{random.randint(0, int(1e10))}/"
                        )
                        if not os.path.exists(final_path):
                            os.mkdir(final_path)
                        policies.get("pol1").export_checkpoint(final_path)
